...
void Game::Loop() {
	while (wnd->isOpen()) {
		...
		CheckCollitions();
		UpdatePhysics();
		DestroyOutOfBoundsCannonBalls(wnd); // Nuevo: Destruir las bolas de cañón fuera de pantalla
		DrawGame();
		wnd->display();
	}
}
...
void Game::DestroyOutOfBoundsCannonBalls(sf::RenderWindow* wnd) {
	for (auto it = cannonBalls.begin(); it != cannonBalls.end(); ) {
		if ((*it)->IsOutOfScreen(wnd)) {
			// Destruir el cuerpo físico
			phyWorld->DestroyBody((*it)->GetBody());
			// Eliminar el objeto CannonBall
			delete (*it);
